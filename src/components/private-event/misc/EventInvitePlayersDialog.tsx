...
export const EventInvitePlayersDialog = ({user, event, onInvite, onClose}: {
    user: User;
    event: PrivateEvent;
    ...
    const handleGroupSelect = (groupId: string) => {
        ...
        const groupMembers = allUsers.filter((u) => group.members.includes(u.username));
            ...
            <div className="overflow-hidden w-11/12 rounded-lg shadow-2xl bg-white p-4 text-black">
                <div className="flex flex-col items-center">
                    ...
                    <div className="flex flex-col gap-8 mt-5 p-4 w-full">
                        ...
                        <div>
                            <label htmlFor="spillere" className="block text-sm font-medium text-gray-700">
                                Enkelte spillere
                            </label>
                            <input
                                type="text"
                                ...
                                <div className="absolute z-10 bg-white w-full border border-black rounded mt-1 max-h-40 overflow-y-auto">
                                    {filteredUsers.length > 0 ? (
                                        filteredUsers.map((member) => (
                                            ...
                                            >
                                                <h1 className="truncate">{member.fullName} ({member.username})</h1>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="px-4 py-2 text-gray-400">Ingen brugere fundet</div>
                                    )}
                                </div>
                            )}
                        </div>
                                ...
                                <div className="mt-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {selectedUsers.map((member) => (
                                            <div
                                                onClick={() => setSelectedUsers(prev => prev.filter((u) => u.id !== member.id))}
                                                key={member.id}
                        ...
                        <div className="flex justify-between">
                            ...
                            <button
                                ...
                                className={`bg-cyan-500 hover:bg-cyan-600 transition duration-300 rounded-lg py-2 px-4 text-white mt-4 ${selectedUsers.length === 0 ? "opacity-50" : ""}`}>
                                Inviter spillere
                            </button>
                        </div>
