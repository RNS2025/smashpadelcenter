...
import communityApi from "../../services/communityService.ts";
        ...
        <>
            ...
            <div className="text-sm hover:bg-gray-700 cursor-pointer">
                {allMatches.map((match) => (
                    <div onClick={() => navigate(`/makkerbørs/${match.id}`)} key={match.id} className="border p-4 rounded-lg space-y-1.5">
                        <h1 className="font-semibold">{format(new Date(match.matchDateTime), "EEEE | dd. MMMM | HH:MM", {locale: da}).toUpperCase()} - 21:00</h1>
                        ...
                        <div className="flex justify-between">
                            ...
                            {[...Array(match.participants.length + match.reservedSpots.length)].map((_, i) => (
                                <UserCircleIcon key={`participant-${i}`} className="h-5 text-cyan-500" />
                            ))}

                            {[...Array(4 - (match.participants.length + match.reservedSpots.length))].map((_, i) => (
                                <UserCircleIcon key={`empty-${i}`} className="h-5 text-gray-500" />
                            ))}
                            </div>
                        </div>
