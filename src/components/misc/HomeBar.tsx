          ...
          <div className="flex items-center gap-8">
            <nav aria-label="Global" className="hidden sm:block">
              <ul className="flex items-center gap-6">
                <li>
                  <h1 onClick={() => navigate("/profil")} className="text-black transition hover:text-gray-500/75 cursor-pointer"> Profil </h1>
                </li>

                {role === "admin" && (
                <li>
                  <h1 onClick={() => navigate("/admin")} className="text-black transition hover:text-gray-500/75 cursor-pointer"> Admin Panel </h1>
                </li>
                )}

                <li>
                  <h1 onClick={handleLogout} className="text-black transition hover:text-gray-500/75 cursor-pointer"> Log ud </h1>
                </li>
              </ul>
            </nav>
        ...
        {dropdownVisible && (
            <div className="absolute right-0 z-10 mt-2 w-48 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
              <div className="py-1" role="none">
                <h1 onClick={() => navigate("/profil")} className="block px-4 py-2 text-sm text-black hover:bg-gray-100">Profil</h1>
                <h1 onClick={() => navigate("/admin")} className="block px-4 py-2 text-sm text-black hover:bg-gray-100">Admin Panel</h1>
                <h1 onClick={handleLogout} className="block px-4 py-2 text-sm text-black hover:bg-gray-100">Log ud</h1>
              </div>
            </div>
        )}
      </header>
